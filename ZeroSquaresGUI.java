...
//                {'.', '.', '.', '.', '.', '.', '.', '.', '.', '.', '.'},
        ...
        for (int row = 0; row < GRID_SIZE; row++) {
            for (int col = 0; col < GRID_SIZE; col++) {
                ...
                buttonGrid[row][col].setFocusable(false); // لمنع التركيز على الزر عند الضغط
        ...
        addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                switch (e.getKeyCode()) {
                    ...
                    case KeyEvent.VK_W:
                        game = game.movePlayer(-1, 0); // استخدم القيمة الجديدة من الحركة
    ...
    private void printPossibleGrid() {
        ...
        for (State state : possibleStates) {
            ...
            for (char[] row : state.getGrid()) { // استخدام `getGrid()` للوصول إلى الشبكة
                for (char cell : row) {
